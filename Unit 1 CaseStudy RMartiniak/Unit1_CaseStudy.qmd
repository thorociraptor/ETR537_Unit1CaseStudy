...
-   Of the variables from Table 1, I would guess that \# Visits to MyGrades tool, Total time online, and \# uses of the 'Search' function may predict final grades. My guess here is that the more someone reviews their grades, looks at course content online, and searches for more information may predict a higher final grade.
...
-   The 3 factors that ultimately predict final grades were the number of postings in discussion forums, mail messages sent, and assessments completed. The model correctly predicted students at risk of failure with 70.3% accuracy, and correctly flagged 80.9% of students who actually ended up failing the course. I debated selecting the number of discussion posts in the question above (though ultimately went with the search function), and I can definitely see this being a different and better indication of learners who are more engaged in their courses--not just searching for information but engaging with the content along with their peers.
...
-   In the log-data file, I was surprised to see a couple of variables. When I think of log data, I expect to see information about what a student did (time spent on a page, what action was taken, what link was clicked, etc.). This file contains information about gender and the reason students were enrolled in the course. I'm curious to find out why.
...
-   Looking at the survey data, I'm unsure about what some of the columns mean. For example, "q1, q2, q3, etc." don't have meaning for me. I wonder if it may be helpful to separate those survey responses out (as I'm assuming they may be Likert scales?). Based on the research question, it may be helpful to extract the columns specifically related to discussion posts and/or (expected) time spent into their own object(s).
...
-   This data would be considered "untidy" because the columns don't seem to make sense with the data presented. Cell A1 looks like it should be the title of the data; and the cell should really read as "Country" given the data in the rest of the column. Additionally, if each variable should be a column and each observation in a row, I'm wondering if the Years (AA1-AH1) should actually be in a column for "year". There are also many empty cells meaning that there's no data for these variables, and it may be worth removing those during analysis.
